...
fn parse_secret_key(secret_string: &str) -> Result<Keypair, String> {
  let decoded_bytes = base58::decode(secret_string)
...
async fn create_new_keypair() -> Json<Value> {
  ...
  let public_key_b58 = base58::encode(new_keypair.pubkey().to_bytes());
  let secret_key_b58 = base58::encode(new_keypair.to_bytes());
...
async fn handle_token_creation(Json(request): Json<TokenCreationRequest>) -> Json<Value> {
  ...
  println!("Mint Authority: {}", request.mint_authority);
  println!("Mint Address: {}", request.mint);
  println!("Decimals: {}", request.decimals);
  ...
  println!("Program ID: {}", mint_instruction.program_id);
  println!("Required Accounts:");
  for (idx, account) in mint_instruction.accounts.iter().enumerate() {
      println!("  [{}] Address: {}, Signer: {}, Writable: {}", 
          idx, account.pubkey, account.is_signer, account.is_writable);
  }
  println!("Instruction Data: {}", base64::encode(&mint_instruction.data));
  println!("===================\n");
...
async fn handle_token_minting(Json(request): Json<TokenMintingRequest>) -> Json<Value> {
  ...
  println!("Program ID: {}", minting_instruction.program_id);
  println!("Required Accounts:");
  for (idx, account) in minting_instruction.accounts.iter().enumerate() {
      println!("  [{}] Address: {}, Signer: {}, Writable: {}", 
          idx, account.pubkey, account.is_signer, account.is_writable);
  }
  println!("Instruction Data: {}", base64::encode(&minting_instruction.data));
  println!("==================\n");
...
async fn sign_user_message(Json(request): Json<MessageSigningRequest>) -> Json<Value> {
  ...
  let signature_b64 = base64::encode(message_signature.as_ref());
  let public_key_b58 = base58::encode(user_keypair.pubkey().to_bytes());
